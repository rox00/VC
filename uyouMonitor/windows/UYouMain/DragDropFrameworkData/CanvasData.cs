...
namespace WpfDragAndDropSmorgasbord.DragDropFrameworkData
{
        ...
        where TObject : UIElement
    {
        ...
        public override void SetData(ref DataObject data) {
            ...
            else if (this.SourceObject is Image)
            {
                Image image = (Image)this.SourceObject;
                if (image.Source != null)
                {
                    textString = image.Source.ToString();
                }
            }
            else if (this.SourceObject is DirectionButton)
            {
                DirectionButton button = (DirectionButton)this.SourceObject;
                if (button.ToolTip != null)
                {
                    textString = button.ToolTip.ToString();
                }
    ...
    public class CanvasDataConsumer<TContainer, TObject> : DataConsumerBase, IDataConsumer
        ...
        where TObject : UIElement
    {
        public delegate void ProcessDelegate(String name, ref Canvas container, ref UIElement srcItem, ref UIElement destItem);
        ProcessDelegate pd;
        public CanvasDataConsumer(string[] dataFormats, Object parent)
            : base(dataFormats)
        {
            KeyMapping keyMapping = parent as KeyMapping;
            pd = new ProcessDelegate(keyMapping.refushDragItem);
        }
            ...
            if(dataProvider != null) {
                ...
                if(dropContainer != null) {
                    if(bDrop) {                        
                        ...
                        try
                        {                            
                            UIElement newItem = new UIElement();
                            Canvas canvas = dropContainer as Canvas;
                            UIElement srcItem = dragSourceObject as UIElement;
                            String name ="";
                            if (dragSourceObject is Image)
                            {
                                name = dragSourceObject.GetValue(Image.NameProperty).ToString();
                            }
                            else
                            {
                                name = dragSourceObject.GetValue(DirectionButton.NameProperty).ToString();
                            }

                            pd(name, ref canvas,ref srcItem, ref newItem);
                            //canvas.Children.Add(rtb2);

                            Point dropPosition = e.GetPosition(canvas);                            
                            Point objectOrigin = dataProvider.StartPosition;
                            Canvas.SetLeft(newItem, dropPosition.X - objectOrigin.X);
                            Canvas.SetTop(newItem, dropPosition.Y - objectOrigin.Y);                                         
