...
{	
	auto &&AimTouchEvents = InputManager::GetInstance().GetInputMethodOperationDevice().m_MouseMappingFunction(
		InputHookHelper::RIGHT_MOUSE_BUTTON, TOUCHTYPE_DOWN);

	if (AimTouchEvents.size() > 0)
	{
		auto iter = AimTouchEvents.begin();
		tagMOUSEPACKET msg = (*iter);
		int x = msg.x;
		int y = msg.y;
...
BOOL MouseOperationHandler::GetFirePoint(POINT &pt)
{
	auto &&FireTouchEvents = InputManager::GetInstance().GetInputMethodOperationDevice().m_MouseMappingFunction(
		InputHookHelper::LEFT_MOUSE_BUTTON,	TOUCHTYPE_DOWN);
...
BOOL MouseOperationHandler::MouseActionHandler(int mouseEvent, int x, int y, TOUCHTYPE Type)
{
	////【zy 2015-12-11 ↓】
	...
	if (mouseEvent == InputHookHelper::RIGHT_MOUSE_BUTTON && Type == TOUCHTYPE_DOWN)
	{	//右键按下
		if(bHasAimPoint)		//如果有瞄准点
		{
			...
			{	//鼠标移动到瞄准点
				...
				InputManager::GetInstance().SendTouchMessage(ptAimDevice.x, ptAimDevice.y, TOUCHTYPE_DOWN, TOUCH_ID_AIM_MAPPING);	//在瞄准镜的地方按下
			}
			else
			{	//关闭瞄准功能，瞄准用的手指弹起
				InputManager::GetInstance().SendTouchMessage(0, 0, TOUCHTYPE_UP, TOUCH_ID_AIM_MAPPING);
				ReleaseCapture();
				ShowCursor(TRUE);
			}
			return TRUE;
	...
	//处理同时按左右键，可能出现长按点的问题
	if(Type != TOUCHTYPE_MOVE)
	{
		if (bHasFirePoint)
		{
			InputManager::GetInstance().SendTouchMessage(ptFireDevice.x, ptFireDevice.y,
				TOUCHTYPE_UP, TOUCH_ID_FIRE_MAPPING);
		}
	}
		
	//鼠标移动，并且瞄准镜是打开的
	if (mouseEvent == InputHookHelper::NULL_MOUSE_BUTTON  && Type == TOUCHTYPE_MOVE && m_bIsAimingOpened)
	{	//进入瞄准状态后的鼠标移动，响应拖到准星操作
		...
		{	//如果不是回到瞄准点的操作
			...
			{	//如果已经移出屏幕范围了
				InputManager::GetInstance().SendTouchMessage(0, 0, TOUCHTYPE_UP, TOUCH_ID_AIM_MAPPING);					//先抬起手指
				InputManager::GetInstance().SendTouchMessage(ptAimDevice.x, ptAimDevice.y, TOUCHTYPE_DOWN, TOUCH_ID_AIM_MAPPING);	//重新到瞄准镜的地方按下
				m_deviceAimPosition.SetPoint(ptAimDevice.x, ptAimDevice.y);

				//把屏幕光标移动回瞄准点，这会再次发出一个MOVE消息，但不会满足(ptAim.x != x || ptAim.y != y)的条件
				SetCursorPos(ptAimScreen.x, ptAimScreen.y);
			}
			InputManager::GetInstance().SendTouchMessage(m_deviceAimPosition.x, m_deviceAimPosition.y, TOUCHTYPE_MOVE, TOUCH_ID_AIM_MAPPING);
		}
	...
	{
		if (Type == TOUCHTYPE_DOWN || Type == TOUCHTYPE_UP)
		{
...
void MouseOperationHandler::OnMouseLeave(WPARAM, LPARAM)
{
	...
	{
		InputManager::GetInstance().SendTouchMessage(0, 0, TOUCHTYPE_UP, TOUCH_ID_MOUSE_CLICK);
	}
}
