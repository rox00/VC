...
void InputHookHelper::Start(HWND hAttachWnd)
{
	...
	DWORD		ProcessID;
	DWORD		ThreadID;
	ThreadID	= GetWindowThreadProcessId(m_hAttachWnd, &ProcessID);
	m_MsgHook	= SetWindowsHookEx(WH_GETMESSAGE, GetMsgProc, NULL, ThreadID);// GetWindowThreadProcessId(hAttachWnd, NULL));
}
