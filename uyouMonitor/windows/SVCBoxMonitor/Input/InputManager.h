...
public:
	...
	public:
		void										SetSendFunction(std::function<void(boost::asio::const_buffer&)> Function)
		{
	...
	private:
		...
		std::function<void(boost::asio::const_buffer&)>		m_SendFunction;
		MAPPING_FUNC_TYPE									m_MappingFunction;
		MAPPING_FUNC_TYPE									m_MouseMappingFunction;
		boost::mutex										m_Mutex;
	};	//InputDevice class

	typedef vector<tagMOUSEPACKET> TouchMessageVector;
...
public:
	void											SendTouchMessage(int x, int y, TOUCHTYPE Action, UINT nFingerId, UINT nDelayTime = 0);
	void											SendKeyMessage(int keyValue, KEYACTIONTYPE Action);
	...
	void											AttachScreenFunc(std::function<void(boost::asio::const_buffer&)> funcTouch, std::function<void(boost::asio::const_buffer&)> funcInput);
	void											DetachScreenFunc();
	void											KeyActionProcess(UINT nVirtualKey, KEYACTIONTYPE nAction);
...
private:
	...
	list<boost::shared_ptr<tagKEYPACKET>>			m_directKeyVec;				////用于暂存按键信息的列表
	boost::shared_mutex								m_directKeyInfoMutex;		////用于同步访问按键信息列表的互斥量
	boost::condition_variable_any					m_condDirectGet;			////同步获取按键信息的条件变量

	list<boost::shared_ptr<tagKEYPACKET>>			m_normalKeytVec;			////用于暂存按键信息的列表
