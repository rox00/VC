...
ScreenZoomHandler::ScreenZoomHandler()
	: m_nScreenWidth(CMainFrame::GetDeviceScreenSize().cx)
{
	m_bIsTouchDown = FALSE;
}

BOOL ScreenZoomHandler::MouseActionHandler(int mouseEvent, int x, int y, TouchEvent::ActionType Type)
{
	...
	case TouchEvent::TOUCH_MOVE:
		...
		if (iDelta && m_bIsTouchDown)
		{
			...
			SendTouchMessage(m_iZoomPosition1, y, m_iZoomPosition2, y, TouchEvent::TOUCH_MOVE, TRUE);
		}
		break;
	case TouchEvent::TOUCH_DOWN:
		CalculateZoomPosition(0);		//初始化
		//SendTouchMessage(m_iZoomPosition1, y, m_iZoomPosition2, y, TouchEvent::TOUCH_DOWN, TRUE);
		m_bIsTouchDown = TRUE;
		break;
...
void ScreenZoomHandler::OnMouseLeave(WPARAM, LPARAM)
{
	if (m_bIsTouchDown)
	{
		...
		SendTouchMessage(0, 0, 0, 0, TouchEvent::TOUCH_UP);
		m_bIsTouchDown = FALSE;
	}
}
...
void ScreenZoomHandler::OnMouseWheel(WPARAM wParam, LPARAM lParam)
{
	...
	{	//如果按下了ctrl键		
		{			
			...
			//按下
			//SendTouchMessage(Point1Begin.x, Point1Begin.y, Point2Begin.x, Point2Begin.y, TouchEvent::TOUCH_DOWN);
			...
			//抬起
			SendTouchMessage(0, 0, 0, 0, TouchEvent::TOUCH_UP);	//抬起的时候不需要指定x和y坐标			
		}
	}
}
...
void ScreenZoomHandler::SendTouchMessage(int x1, int y1, int x2, int y2, TouchEvent::ActionType Action, BOOL bIsMapping)
{
	if (bIsMapping)
	{
		int x = 0;
