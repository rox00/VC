...
InputManager::TouchMessageVector KeyboardMapper::MouseMappingFunction(UINT nVirtualKey, UINT nAction)
{
	...
	{
		GenereateTouchMessage(TouchMessages, ConfigItem, (TouchEvent::ActionType)nAction);
	}
...
InputManager::TouchMessageVector KeyboardMapper::KeyMappingFunction(UINT nVirtualKey, UINT nAction)
{
	...
	if(ConfigItem.m_Action == MappingConfigItem::ACTION_SWIPE)
	{
		...
		{	//WM_KEYUP
			...
			{	//弹起的是之前按下的键
				...
				if(m_KeyDownList.empty())
				{	//不剩下其他键了，手指头弹起
					GenereateTouchMessage(TouchMessages, ConfigItem, TouchEvent::TOUCH_UP);
				}
				else
				{	//滑回上上次按下的键
	...
	{	//Tap操作，比较简单
		GenereateTouchMessage(TouchMessages, ConfigItem, (nAction == WM_KEYDOWN) ? 
			TouchEvent::TOUCH_DOWN : TouchEvent::TOUCH_UP);
	}
...
void KeyboardMapper::GenereateTouchMessage(InputManager::TouchMessageVector& rMessages, 
	MappingConfigItem &rConfigItem, TouchEvent::ActionType nAction)
{
	TouchEvent::Message TouchMsg;

	TouchMsg.set_x(rConfigItem.m_Position.x);
	TouchMsg.set_y(rConfigItem.m_Position.y);
	TouchMsg.set_actiontype(nAction);
	TouchMsg.set_delaytime(0);
	TouchMsg.set_id(rConfigItem.m_nFingerId);
	...
	UINT nFingerId, CPoint ptStart, CPoint ptStop, BOOL bNeedTouchDown)
{
	...
	if(bNeedTouchDown)
	{
		TouchMsg.set_x(ptStart.x);
		TouchMsg.set_y(ptStart.y);
		TouchMsg.set_actiontype(TouchEvent::TOUCH_DOWN);
		TouchMsg.set_delaytime(SWIPE_DELAY_TIME);
	...
	auto pGenerateMoveEventFunction = [&](int iJudgeComponent)
	{
		...
		TouchMsg.set_x(x);
		TouchMsg.set_y(y);
		TouchMsg.set_actiontype(TouchEvent::TOUCH_MOVE);
		TouchMsg.set_delaytime(SWIPE_DELAY_TIME);	//滑动过程中有少量延迟是比较合理的
		TouchMsg.set_id(nFingerId);
